...
export class ExperienciaComponent implements OnInit {
  ...
  constructor(public auth: AuthService, private service: ExperienciaService, private dialog: MatDialog) { }
  ...
  delete(item: Experiencia){
    if (confirm(`Seguro que desea borrar la entrada ${item.nombre}?`))
      this.service.deleteExperiencia(item.id).subscribe(() => window.location.reload())
  }
...
export class ExperienciaCreate {
  ...
  exp: Experiencia = new Experiencia(0,'','','',false,[],'',new Date(),new Date());
  competencias: string = '';

  constructor(private service: ExperienciaService, private dialogRef: MatDialogRef<ExperienciaCreate>, @Inject(MAT_DIALOG_DATA) private data?: Experiencia){
    if (data){
  ...
  onSubmit(event: Event){
    ...
    if (this.data){
      this.service.editExperiencia(this.exp.id,this.exp).subscribe(() => { this.dialogRef.close(); window.location.reload(); })
    } else
      this.service.addExperiencia(this.exp).subscribe(() => { this.dialogRef.close(); window.location.reload(); })
  }
