...
export class ContactoComponent implements OnInit {
  ...
  constructor(private service: ContactoService, public auth: AuthService, private dialog: MatDialog) { }
  ...
  delete(item: Contacto){
    if (confirm(`Seguro que desea borrar la entrada ${item.nombre}?`))
      this.service.deleteContacto(item.id).subscribe(() => window.location.reload())
  }
...
export class ContactoCreate {
  ...
  constructor(private service: ContactoService, private dialogRef: MatDialogRef<ContactoCreate>, @Inject(MAT_DIALOG_DATA) private data?: Contacto){
    if (data) {
      this.contacto = data;
  ...
  onSubmit(event: Event){
    ...
    if (this.data) {
      this.service.editContacto(this.contacto.id, this.contacto).subscribe(() => { this.dialogRef.close(); window.location.reload() });
    } else 
      this.service.addContacto(this.contacto).subscribe(() => { this.dialogRef.close(); window.location.reload() });
  }
