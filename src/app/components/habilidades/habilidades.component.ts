...
export class HabilidadesComponent implements OnInit {
  ...
  constructor(private service: HabilidadesService, public auth: AuthService, public dialog: MatDialog) {
  ...
  delete(item: Habilidad){
    if (confirm(`Seguro que desea borrar la entrada ${item.nombre}?`)){
      this.service.deleteHabilidad(item.id).subscribe(() => window.location.reload());
    }
  }
...
export class HabilidadesCreate {
  ...
  constructor(private service: HabilidadesService, public dialogRef: MatDialogRef<HabilidadesCreate>, @Inject(MAT_DIALOG_DATA) private data?: Habilidad){
    if (data) {
      this.skill = data;
  ...
  onSubmit(event: Event){
    ...
    if (this.data){
      this.service.editHabilidad(this.skill.id, this.skill).subscribe(() => { this.dialogRef.close(); window.location.reload(); });
    } else {
      this.service.addHabilidad(this.skill).subscribe(() => { this.dialogRef.close(); window.location.reload(); });
    }
  }
